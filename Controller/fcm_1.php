...
/**
 * Description of fcmClass
 *
 * @author Fikri Utri Amri
 */
class fcm {
...
//        $this->eValue = $attr['fo']; //0.0001; //10 pangkat -4===0.0001
        $this->eValue = 0.0001; //0.0001; //10 pangkat -4===0.0001
        $this->pValue = 0;
        $this->nData = intval($attr['n']); //banyak data
        //Bila wxh sama pada setiap gambar
        if (!isset($_SESSION['UvalueAcak'])) {
            //set nilai awal partisi uij secara acak
            $_SESSION['UvalueAcak'] = $this->SetUAcak();
        } else if (isset($_SESSION['UvalueAcak'])) {
            if (count($_SESSION['UvalueAcak']) == ($this->nData) && count($_SESSION['UvalueAcak'][0]) == $this->cValue) {
